import{_ as n,c as a,o as s,a4 as p}from"./chunks/framework.4aTu-Nia.js";const b=JSON.parse('{"title":"一、基本概念","description":"","frontmatter":{},"headers":[],"relativePath":"docs/01 学习/专栏/Golang/Go 源码/Go 源码 | Go channel 的底层实现原理.md","filePath":"docs/01 学习/专栏/Golang/Go 源码/Go 源码 | Go channel 的底层实现原理.md"}'),e={name:"docs/01 学习/专栏/Golang/Go 源码/Go 源码 | Go channel 的底层实现原理.md"},l=p(`<h1 id="一、基本概念" tabindex="-1">一、基本概念 <a class="header-anchor" href="#一、基本概念" aria-label="Permalink to &quot;一、基本概念&quot;">​</a></h1><p>Go 中的 channel 是一个队列，遵循先进先出的原则，负责协程之间的通信（Go 语言提倡不要通过共享内存来通信，而要通过通信来实现内存共享，CSP(Communicating Sequential Process)并发模型，就是通过 goroutine 和 channel 来实现的）</p><p>使用场景：</p><p>1、停止信号监听</p><p>2、定时任务</p><p>3、生产方和消费方解耦</p><p>4、控制并发数</p><h1 id="二、底层数据结构" tabindex="-1">二、底层数据结构 <a class="header-anchor" href="#二、底层数据结构" aria-label="Permalink to &quot;二、底层数据结构&quot;">​</a></h1><p>通过 var 声明或者 make 函数创建的 channel 变量是一个存储在函数栈帧上的指针，占用 8 个字节，指向堆上的 hchan 结构体</p><p>源码包中<code>src/runtime/chan.go</code>定义了 hchan 的数据结构：</p><p><img src="https://cdn.jsdelivr.net/gh/caijinlin/imgcdn/hchan.png#id=uCG0j&amp;originHeight=773&amp;originWidth=1080&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;status=done&amp;style=none&amp;title=" alt=""></p><h2 id="一-hchan-结构体" tabindex="-1">一）hchan 结构体 <a class="header-anchor" href="#一-hchan-结构体" aria-label="Permalink to &quot;一）hchan 结构体&quot;">​</a></h2><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>type hchan struct {</span></span>
