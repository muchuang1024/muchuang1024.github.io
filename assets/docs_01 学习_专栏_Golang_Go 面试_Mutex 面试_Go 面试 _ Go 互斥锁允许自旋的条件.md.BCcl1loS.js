import{_ as n,c as s,o as a,a4 as p}from"./chunks/framework.4aTu-Nia.js";const m=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"docs/01 学习/专栏/Golang/Go 面试/Mutex 面试/Go 面试 | Go 互斥锁允许自旋的条件.md","filePath":"docs/01 学习/专栏/Golang/Go 面试/Mutex 面试/Go 面试 | Go 互斥锁允许自旋的条件.md"}'),e={name:"docs/01 学习/专栏/Golang/Go 面试/Mutex 面试/Go 面试 | Go 互斥锁允许自旋的条件.md"},t=p(`<p><strong>线程没有获取到锁时常见有 2 种处理方式</strong></p><p>1、一种是没有获取到锁的线程就一直循环等待判断该资源是否已经释放锁，这种锁也叫做<strong>自旋锁</strong>，它不用将线程阻塞起来， 适用于并发低且程序执行时间短的场景，缺点是 cpu 占用较高</p><p>2、另外一种处理方式就是把自己阻塞起来，会<strong>释放 CPU 给其他线程</strong>，内核会将线程置为「睡眠」状态，等到锁被释放后，内核会在合适的时机唤醒该线程，适用于高并发场景，缺点是有线程上下文切换的开销</p><p>Go 语言中的 Mutex 实现了自旋与阻塞两种场景，当满足不了自旋条件时，就会进入阻塞</p><p><strong>允许自旋的条件</strong></p><p>1、锁已被占用，并且锁不处于饥饿模式。</p><p>2、积累的自旋次数小于最大自旋次数（active_spin=4）。</p><p>3、cpu 核数大于 1。</p><p>4、有空闲的 P。</p><p>5、当前 goroutine 所挂载的 P 下，本地待运行队列为空。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>if old&amp;(mutexLocked|mutexStarving) == mutexLocked &amp;&amp; runtime_canSpin(iter) {</span></span>
