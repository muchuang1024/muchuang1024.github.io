...
<span class="line"><span></span></span>
<span class="line"><span>source ~/.bash_profile</span></span></code></pre></div><h1 id="五、进入" tabindex="-1">五、进入 <a class="header-anchor" href="#五、进入" aria-label="Permalink to &quot;五、进入&quot;">​</a></h1><p>选择你喜欢的主题 + Enter 确认安全须知 + Enter 使用默认 Terminal 配置 + Enter 信任工作目录 + Enter</p><p>符号说明： $$ \\begin{array}{l} I：司机可以卸车的候选点位集合, i \\in I 表示一个候选点位 \\[6pt] Q：目标卸车数 \\[6pt] x_{i} \\in {0,1} : 是否选中点位 i \\[6pt] q_{i}: 点位 i 的卸车数量 \\[6pt] d(i​,j​): 两点 i 和 j​ 之间的距离 \\[6pt] p_{driver}: 司机当前位置 \\[6pt] p_{i}: 点位 i 位置 \\[6pt] T_{i}: 点位 i 周转 \\[6pt] L_{i}: 点位 i 缺口 \\[6pt] N_{max}: 访问点位数量的最大限制 \\end{array} $$</p><p>决策变量：</p><p>$$ \\begin{array}{l} x_{i}: 表示点位 i 是否被选中作为卸车点，选中为1， x_{i} \\in {0, 1} \\[6pt] q_{i}: 点位 i 卸车数 \\[6pt] \\end{array} $$ 目标函数：</p><p>$$ \\begin{array}{l} \\textbf{主目标：最大化调度收益} \\max f = \\sum_{i \\in I} x_i \\cdot T_i \\cdot q_{i} \\[6pt] \\textbf{次目标1：最小化调度成本} \\min g = D(p_{}{\\text{Location}(p_{driver})}, \\text{Location}(p_1)) + \\sum_{j=1}^{k-1} D(\\text{Location}(p_j), \\text{Location}(p_{j+1})) \\[6pt] \\textbf{次目标2：最小化点位数量} \\min h = \\sum_{i \\in I} x_i \\[6pt] 惩罚项: P_1 \\cdot \\max\\left(0, \\sum_{i \\in I} x_i - 3\\right) \\end{array} $$</p><p>约束条件：</p><p>$$ \\begin{array}{l} \\textbf{访问点位数量约束: } \\sum_{i \\in I} x_i \\le N_{\\max_visit} \\[6pt] \\textbf{单点位卸车数量约束: } q_i \\le Q_{\\text{available}_i} \\cdot x_i \\quad \\forall i \\in I \\[6pt] \\textbf{总卸车数量约束: } \\sum_{i \\in I} q_i = Q_{\\text{target}} \\[6pt] \\end{array} $$</p>`,15),t=[i];function l(o,c,r,d,_,h){return n(),s("div",null,t)}const b=a(e,[["render",l]]);export{m as __pageData,b as default};
