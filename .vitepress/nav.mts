// export default [
//   {
//     text: '学习',
//     items: [
//       { text: '书单', link: 'docs/01 学习/书单/写作类书单.md' },
//       { text: '提示词', link: 'docs/01 学习/提示词/自媒体文章拆解.md' },
//       { text: '专栏', link: 'docs/01 学习/专栏/Golang/分销手册.md' },
//       { text: '付费', link: 'docs/01 学习/付费/小册.md' }
//     ]
//   },
//   {
//     text: '工作',
//     link: 'docs/02 工作/物流调度/调度算法.md'
//   },
//   {
//     text: '博客',
//     items: [
//       { text: '教程', link: 'docs/03 博客/教程/VitePress 教程.md' },
//       { text: '开发', link: 'docs/03 博客/开发/自动生成目录.md' },
//       { text: '工具', link: 'docs/03 博客/效率提升/常用工具.md' }
//     ]
//   },
//   {
//     text: '付费产品',
//     link: 'docs/01 学习/付费/小册.md'
//   }
// ]

export default [
  { text: 'AI 提示词', link: 'docs/01 AI/AI 提示词/写作/推荐爆款标题.md' },
  { text: 'AI 提效', link: 'docs/01 AI/AI 提效/大纲.md' },
  { text: 'AI 教程', link: 'docs/01 AI/AI 教程/Coze 入门指南.md' },

  { text: '供应链技术', link: 'docs/02 技术/供应链技术/调度算法.md' },
  { text: '算法', link: 'docs/02 技术/算法/刷题方法.md' },
  { text: 'Golang', link: 'docs/02 技术/Golang/大纲.md' },
  { text: '效率提升', link: 'docs/02 技术/效率工具/常用工具.md' }

  // { text: '我的产品', link: 'docs/03 产品/木川/小册.md' }
]
